...
const Users = () => {
    ...
    const buttonToLoadMore = data.getUsers.users.length >= 1 ? <Button variant='contained' style={{backgroundColor: '#15639d', color: '#ffffff'}} onClick={ async () => {await fetchMore({variables: {after: data.getUsers.users[data.getUsers.users.length - 1].id}})}}>Cargar más</Button> : null;
